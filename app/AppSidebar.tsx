...
function SidebarNavItem({ href, label, icon, active, feature, compact, onClick }: { href: string; label: string; icon: ReactNode; active: boolean; feature: "create" | "fees" | "openbid"; compact: boolean; onClick?: MouseEventHandler<HTMLAnchorElement> }) {
  ...
  return (
    <Link
      ...
      className={`group relative isolate flex w-full items-center rounded-lg py-1.5 text-left text-sm text-white/68 transition-[background-color,color,box-shadow] duration-300 ${compact ? "justify-center px-1 min-[1800px]:justify-start min-[1800px]:gap-2 min-[1800px]:px-2.5" : "gap-2 px-2.5"} ${active ? "" : hoverTone}`}
    ...
    >
      ...
      <span className={compact ? "sr-only min-[1800px]:not-sr-only" : ""}>{label}</span>
      {compact ? <span className="pointer-events-none absolute left-[calc(100%+9px)] top-1/2 z-50 -translate-y-1/2 whitespace-nowrap rounded-lg border border-white/[0.09] bg-[#111513]/96 px-2.5 py-1.5 text-[10px] font-medium text-white/72 opacity-0 shadow-[0_12px_28px_rgba(0,0,0,0.42)] backdrop-blur-xl transition duration-150 group-hover:translate-x-0.5 group-hover:opacity-100 min-[1800px]:hidden">{label}</span> : null}
    </Link>
  );
}

export default function AppSidebar({ onOpenCollectFees, collectFeesOpen = false, compact = false, collapsible = false, onToggleCompact }: { onOpenCollectFees: () => void; collectFeesOpen?: boolean; compact?: boolean; collapsible?: boolean; onToggleCompact?: () => void }) {
  const pathname = usePathname();

  return (
    <aside className={`sticky top-14 z-[245] hidden h-[calc(100vh-6.25rem)] shrink-0 flex-col border-r border-white/[0.08] bg-[linear-gradient(180deg,#0b0c0c_0%,#090a0a_100%)] shadow-[8px_0_32px_rgba(0,0,0,0.12)] transition-[width] duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] md:flex ${compact ? "w-[66px] min-[1800px]:w-[272px]" : "w-[272px]"}`}>
      {collapsible ? (
        <button
          type="button"
          onClick={onToggleCompact}
          aria-label={compact ? "Open sidebar" : "Collapse sidebar"}
          className="absolute right-0 top-0 z-50 grid h-8 w-6 -translate-y-1/2 translate-x-1/2 place-items-center bg-transparent text-white/34 outline-none transition-colors duration-180 hover:text-[#52dfb2] focus-visible:text-[#52dfb2] min-[1800px]:hidden"
        >
          {compact ? <ChevronRight className="h-3.5 w-3.5" strokeWidth={1.65} /> : <ChevronLeft className="h-3.5 w-3.5" strokeWidth={1.65} />}
        </button>
      ) : null}
      <div className={`app-sidebar-scroll min-h-0 flex-1 overflow-y-auto pb-6 pt-3 ${compact ? "px-2 min-[1800px]:px-3 min-[1800px]:pr-[6px]" : "px-3 pr-[6px]"}`}>
        {compact ? (
          <>
            <Link href="/?search=1" aria-label="Search" className="group relative grid h-10 w-full place-items-center rounded-xl border border-white/[0.085] bg-white/[0.025] text-white/42 outline-none transition hover:border-[#18c98e]/24 hover:bg-[#18c98e]/[0.055] hover:text-[#18c98e] focus-visible:border-[#18c98e]/38 min-[1800px]:hidden"><Search className="h-4 w-4" /><span className="pointer-events-none absolute left-[calc(100%+9px)] top-1/2 z-50 -translate-y-1/2 whitespace-nowrap rounded-lg border border-white/[0.09] bg-[#111513]/96 px-2.5 py-1.5 text-[10px] font-medium text-white/72 opacity-0 shadow-[0_12px_28px_rgba(0,0,0,0.42)] backdrop-blur-xl transition duration-150 group-hover:translate-x-0.5 group-hover:opacity-100">Search</span></Link>
            <form action="/" className="relative hidden min-[1800px]:block">
        ...
        <nav className={`mt-3 ${compact ? "space-y-1 min-[1800px]:space-y-2" : "space-y-2"}`} aria-label="Primary navigation">
        ...
        <div className={`mt-5 items-center justify-between px-1 ${compact ? "hidden min-[1800px]:flex" : "flex"}`}>
        ...
        <div className={compact ? "mt-4 space-y-1 min-[1800px]:mt-2 min-[1800px]:space-y-0.5" : "mt-2 space-y-0.5"}>
          {boards.map((board, index) => (
            ...
            >
              ...
              <span className={compact ? "sr-only min-[1800px]:not-sr-only" : "truncate"}>{board.name}</span>
              {compact ? <span className="pointer-events-none absolute left-[calc(100%+9px)] top-1/2 z-50 -translate-y-1/2 whitespace-nowrap rounded-lg border border-white/[0.09] bg-[#111513]/96 px-2.5 py-1.5 text-[10px] font-medium text-white/72 opacity-0 shadow-[0_12px_28px_rgba(0,0,0,0.42)] backdrop-blur-xl transition duration-150 group-hover:translate-x-0.5 group-hover:opacity-100 min-[1800px]:hidden">{board.name}</span> : null}
            </Link>
          ))}
        </div>
      </div>
